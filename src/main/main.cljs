...
(def auth-url "https://github.com/login/oauth/authorize?login=\"\"&client_id=3056f749c2e91ce4d780&scope=admin:org delete_repo")
...
;;(.show @auth-window)
(.loadURL @auth-window auth-url)
(.on @auth-window "closed" #(reset! main-window nil))
(.webContents.on @auth-window  "will-navigate" #(get-token %2)))
